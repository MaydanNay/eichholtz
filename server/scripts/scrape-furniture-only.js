#!/usr/bin/env node
/**
 * scrape-furniture-only.js v2
 * Парсит ВСЕ ~1266 товаров категории Мебель (Furniture) с https://www.eichholtz.com/en/
 * С интеллектуальным разрешением категорий (приоритет более специфичным подкатегориям, таким как Модульные диваны).
 */
...
// Приоритетная карта категорий Algolia -> Название в нашей БД
// Первые в списке = самые специфичные категории!
const CATEGORY_PRIORITY = [
  // ── Специфичные Диваны & Пуфики ──────────────────────────────────────────────
...
// Выбираем САМУЮ СПЕЦИФИЧНУЮ категорию из всех категорий товара по списку приоритетов
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════')
  console.log('  Eichholtz Furniture (Мебель) Scraper v2')
  console.log('═══════════════════════════════════════════════════════════\n')
  ...
  // 3. Запрос товаров — собираем все уникальные хиты
  console.log('\n[3/4] Fetching Furniture products by subcategory...')
  const uniqueHits = new Map() // objectID -> hit

  for (const item of CATEGORY_PRIORITY) {
    const catPath = item.path
    try {
      const { hits, totalOnSite } = await fetchSubcategoryProducts(catPath)
      let added = 0
      for (const h of hits) {
      ...
      console.log(`  [${totalOnSite} на сайте | +${added} новых] ${item.dbName}`)
    } catch (err) {
  ...
  console.log(`\n  Всего уникальных товаров Мебели получено: ${uniqueHits.size}`)

  // 4. Назначаем наиболее специфичные категории и вставляем/обновляем в БД
  ...
  // Итоговая статистика по Мебели
  console.log('\n📊 Итоговая статистика товаров Мебели в БД по подкатегориям:')
  const { rows: stats } = await query(`
    SELECT c.name, COUNT(p.id) as n
    FROM categories c
    LEFT JOIN products p ON p.category_id = c.id
    WHERE c.parent_id IN (SELECT id FROM categories WHERE parent_id = (SELECT id FROM categories WHERE name = 'Мебель'))
       OR c.id = (SELECT id FROM categories WHERE name = 'Ковры | Ковровые покрытия')
  ...
  console.log(`\nВсего товаров в категории Мебель: ${total.n}`)
