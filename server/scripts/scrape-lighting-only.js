#!/usr/bin/env node
/**
 * scrape-lighting-only.js
 * Парсит ВСЕ ~663 товара категории Освещение (Lighting) с https://www.eichholtz.com/en/
 * Сопоставляет с подкатегориями в БД (Люстры, Бра, Настольные лампы, Торшеры и т.д.)
 */
...
async function buildCategoryMap() {
  ...
  const map = {}
  for (const item of LIGHTING_CATEGORIES) {
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════')
  console.log('  Eichholtz Lighting (Освещение) Scraper v1')
  console.log('═══════════════════════════════════════════════════════════\n')
  ...
  // 3. Запрос товаров
  console.log('\n[3/4] Fetching Lighting products by category...')
  ...
  console.log(`\n  Всего уникальных товаров Освещения получено: ${uniqueHits.size}`)
  ...
  // Итоговая статистика по Освещению
  console.log('\n📊 Итоговая статистика товаров Освещения в БД по подкатегориям:')
  const { rows: stats } = await query(`
    SELECT c.name, COUNT(p.id) as n
    FROM categories c
    LEFT JOIN products p ON p.category_id = c.id
    WHERE c.parent_id = (SELECT id FROM categories WHERE name = 'Освещение')
  ...
  const { rows: [total] } = await query(`
    WITH RECURSIVE cat_tree AS (
      SELECT id FROM categories WHERE name = 'Освещение'
      UNION ALL
  ...
  console.log(`\nВсего товаров в категории Освещение: ${total.n}`)
