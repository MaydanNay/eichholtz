#!/usr/bin/env node
/**
 * scrape-outdoor-only.js
 * Парсит ВСЕ ~208 товаров категории Для улицы (Outdoor) с https://www.eichholtz.com/en/
 * Сопоставляет с подкатегориями в БД (Кресла для улицы, Уличные диваны, Кофейные столики на открытом воздухе и т.д.)
 */
...
const OUTDOOR_CATEGORIES = [
  ...
  // ── Специфичные Диваны & Кровати для улицы ─────────────────────────────────
  ...
  { path: 'Collection /// Outdoor /// Outdoor covers',                          dbName: 'Чехлы для улицы' },
]
...
async function buildCategoryMap() {
  ...
  const map = {}
  for (const item of OUTDOOR_CATEGORIES) {
...
async function fetchCategoryProducts(categoryPath) {
  ...
  const isLevel2 = !categoryPath.includes(' /// Outdoor ') || categoryPath.endsWith('accessories') || categoryPath.endsWith('lighting') || categoryPath.endsWith('covers') || categoryPath.endsWith('carpets') || categoryPath.endsWith('rugs')
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════')
  console.log('  Eichholtz Outdoor (Для улицы) Scraper v1')
  console.log('═══════════════════════════════════════════════════════════\n')
  ...
  // 3. Запрос товаров
  console.log('\n[3/4] Fetching Outdoor products by category...')
  ...
  console.log(`\n  Всего уникальных товаров Для улицы получено: ${uniqueHits.size}`)
  ...
  // Итоговая статистика по категории Для улицы
  console.log('\n📊 Итоговая статистика товаров Для улицы в БД по подкатегориям:')
  const { rows: stats } = await query(`
    WITH RECURSIVE cat_tree AS (
      SELECT id FROM categories WHERE name = 'Для улицы'
      UNION ALL
  ...
  console.log(`\nВсего товаров в категории Для улицы: ${total.n}`)
