...
async function run() {
  console.log("=== СИНХРОНИЗАЦИЯ КАТЕГОРИИ АКСЕССУАРЫ (747 ТОВАРОВ 1-В-1) ===");
  ...
  console.log(`Уникальных товаров в Accessories из Algolia: ${uniqueHits.length}`);

  const { rows: treeRows } = await query(`
    WITH RECURSIVE cat_tree AS (
      SELECT id FROM categories WHERE id = 595
      UNION ALL
  ...
  for (const h of uniqueHits) {
    ...
    if (rows.length > 0) {
      ...
      if (!validCatIds.has(p.category_id)) {
        await query(`UPDATE products SET category_id = 595 WHERE id = $1`, [p.id]);
        updatedCount++;
      }
    }
  }
  ...
  console.log(`[Аксессуары] Привязано недостающих: ${updatedCount}. Новый итог в БД: ${finalDb[0].total} (в оригинале ${uniqueHits.length})`);
