...
export default function DesignersInquiryModal({ isOpen, onClose, source }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const message = [
      source ? `Источник: ${source}` : 'Заявка в программу лояльности для дизайнеров',
    ...
    try {
      ...
      openInquiryWhatsApp({ name, phone, message, kind: 'designers' })
      setSent(true)
      setForm({ ...EMPTY_FORM, workFormat: DESIGNER_WORK_FORMATS[0] })
    } catch (err) {
