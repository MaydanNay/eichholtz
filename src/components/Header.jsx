...
export default function Header({
  ...
  const openCategoryRoot = openMegaMenu?.startsWith('category:')
    ? openMegaMenu.slice('category:'.length)
    : null

  const megaMenuControlsId = (megaMenu) => {
    if (megaMenu === 'collections') return 'collections-menu'
    if (megaMenu?.startsWith('category:')) {
      return `category-menu-${megaMenu.slice('category:'.length).toLowerCase().replace(/\s+/g, '-')}`
    }
    return undefined
  }
  ...
  return (
    <>
      ...
      <div className="header__top">
        ...
        >
          <img src="/logo.svg" alt="Eichholtz" className="header__logo-img" />
        </button>

        <div className="header__top-right">
          <nav className="header__top-links" aria-label="Дополнительная навигация">
            {TOP_LINKS.map((item) => (
              <button
                key={item.id}
                type="button"
                className={`header__top-link${activePage === item.id ? ' header__top-link--active' : ''}`}
                onClick={() => {
                  closeMegaMenu()
                  onNavigate(item.id)
                }}
      ...
      />
      {CATEGORY_NAV_ROOTS.map((rootName) => (
        <CategoryNavPanel
          key={rootName}
          rootName={rootName}
          isOpen={openCategoryRoot === rootName}
          onClose={closeMegaMenu}
        />
      ))}
