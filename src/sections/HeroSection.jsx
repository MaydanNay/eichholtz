    ...
    <section className={`hero${entered ? ' hero--entered' : ''}`}>
      ...
      <div className="hero__overlay">
        {current && (
          <>
            {current.season_name && (
              <p key={`${current.id}-season`} className="hero__season hero__title--animate">
                {current.season_name}
              </p>
            )}
            <h1 key={current.id} className="hero__title hero__title--animate" style={{ display: 'flex', alignItems: 'center', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              {current.name}
              {current.is_new && (
