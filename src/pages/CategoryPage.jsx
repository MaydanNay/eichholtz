...
export default function CategoryPage({ categoryId, onCartOpen }) {
  ...
  return (
    <div className="collection-page">
      <Reveal className="collection-page__hero" variant="blur-up">
        {displayCategory?.image_url && (
          <div className="collection-page__hero-media">
            <img 
            src={displayCategory.image_url || '/logo.webp'} 
            alt="" 
            className={!displayCategory.image_url ? 'img-fallback' : ''}
          />
          </div>
        )}
        <div className="collection-page__hero-content">
          ...
          <div className="collection-page__stats" style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem', fontSize: '0.85rem', color: 'var(--color-gray-text)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          ...
          <div className="section-heading section-heading--left" style={{ width: '100%', marginTop: '1.5rem' }}></div>
      ...
      {subcategories.length > 0 && (
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', padding: '2rem 5% 2rem', justifyContent: 'center' }}>
          <button
            onClick={() => parentCategory ? navigate(categoryUrl(parentCategory)) : {}}
            style={{
              background: !parentCategory ? '#000' : 'var(--color-bg-alt, #f9f9f9)',
              border: !parentCategory ? '1px solid #000' : '1px solid #eaeaea',
              borderRadius: '30px',
              padding: '0.75rem 1.5rem',
              cursor: !parentCategory ? 'default' : 'pointer',
              fontSize: '0.95rem',
              color: !parentCategory ? '#fff' : 'var(--color-text)',
          ...
          {subcategories.map(sub => {
            ...
            return (
              <button
                ...
                style={{
                  background: isActive ? '#000' : 'var(--color-bg-alt, #f9f9f9)',
      ...
      <ProductsCatalogSection
        ...
        titleOverride={`Товары: ${category.name}`}
        hideFilterChip
        hideSidebar
        sidebarNavigates
