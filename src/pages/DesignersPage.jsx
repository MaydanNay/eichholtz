...
export default function DesignersPage() {
  ...
  const [formSource, setFormSource] = useState('Страница дизайнеров')

  useEffect(() => {
    if (location.hash === '#designers-form') {
      setFormSource('Страница дизайнеров (по ссылке из подвала)')
      setIsFormOpen(true)
    }
  }, [location.hash])

  const openForm = (source) => {
    setFormSource(typeof source === 'string' ? source : 'Страница дизайнеров (нижняя кнопка)')
    setIsFormOpen(true)
  }
  ...
  return (
    <div className="designers-page">
      ...
      <Reveal as="section" className="designers-intro" variant="up">
        ...
        <div className="designers-intro__cta" style={{ marginTop: '2.5rem', textAlign: 'center' }}>
          <p style={{ marginBottom: '1.25rem', fontSize: '1.125rem', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto', lineHeight: '1.4' }}>
            Заполните форму (имя и контакты), и мы вышлем вам всю 3D-библиотеку
          </p>
          <button type="button" className="designers-form__open-btn" onClick={() => openForm('Заявка на 3D-библиотеку (После вступления)')}>
            Получить 3D-библиотеку
          </button>
        </div>
      </Reveal>
      ...
      <DesignersInquiryModal isOpen={isFormOpen} onClose={closeForm} source={formSource} />
    </div>
  )
}
