          ...
          <div className="account-page__inner">
            ...
            <div className="account-page__tabs" style={{ display: 'flex', gap: '1.5rem', marginBottom: '2rem', borderBottom: '1px solid var(--color-gray-light)' }}>
              <button 
                ...
                style={{ padding: '0.5rem 0', fontWeight: activeTab === 'orders' ? '500' : '400', borderBottom: activeTab === 'orders' ? '2px solid var(--color-black)' : '2px solid transparent', color: activeTab === 'orders' ? 'var(--color-black)' : 'var(--color-gray-text)' }}
              >
                Мои заказы и заявки
              </button>
              <button 
                ...
                style={{ padding: '0.5rem 0', fontWeight: activeTab === 'favorites' ? '500' : '400', borderBottom: activeTab === 'favorites' ? '2px solid var(--color-black)' : '2px solid transparent', color: activeTab === 'favorites' ? 'var(--color-black)' : 'var(--color-gray-text)' }}
              >
                Мои избранные
              </button>
            </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                    {favorites.map((product) => (
                      <Link key={product.id} to={productUrl(product)} style={{ display: 'block', textDecoration: 'none', color: 'inherit', transition: 'opacity 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.opacity = '0.8'} onMouseLeave={(e) => e.currentTarget.style.opacity = '1'}>
                        <div style={{ aspectRatio: '1', backgroundColor: '#f9f9f9', marginBottom: '0.8rem', borderRadius: '4px', overflow: 'hidden' }}>
                          {product.image_url ? (
                        ...
                        <p style={{ fontSize: '0.85rem', marginBottom: '0.2rem', color: 'var(--color-gray-text)' }}>
