          ...
          try {
            ...
            if (migrated.socials) {
              migrated.socials = migrated.socials.map(s => {
                if (s.iconUrl && s.iconUrl.includes('%3Csvg') && s.iconUrl.includes('#ffffff')) {
      ...
      <div className="admin-page__content">
        {!isEditing ? (
          <div className="admin-form" style={{ maxWidth: '800px' }}>
            <h2 style={{ fontSize: '1.15rem', marginBottom: '1rem', fontWeight: 500, paddingBottom: '0.5rem', borderBottom: '1px solid #eee' }}>
              Шоурумы и телефоны
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
              <div>
                <strong style={{ display: 'block', fontSize: '0.9rem', color: '#888', marginBottom: '0.25rem' }}>Астана</strong>
              ...
              <div>
                <strong style={{ display: 'block', fontSize: '0.9rem', color: '#888', marginBottom: '0.25rem' }}>Алматы</strong>
            ...
            <div style={{ marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              <div>
                <strong style={{ display: 'block', fontSize: '0.9rem', color: '#888', marginBottom: '0.25rem' }}>Общая почта</strong>
                <p style={{ margin: 0 }}>{contacts.emailGeneral || 'Не указана'}</p>
              </div>
              <div>
                <strong style={{ display: 'block', fontSize: '0.9rem', color: '#888', marginBottom: '0.25rem' }}>Сотрудничество и предложения</strong>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {contacts.socials?.length ? contacts.socials.map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  {s.iconUrl ? (
                    <img src={s.iconUrl} alt={s.name} style={{ width: '32px', height: '32px', objectFit: 'contain', background: '#333', borderRadius: '4px', padding: '4px' }} />
                  ) : (
                    <div style={{ width: '32px', height: '32px', background: '#eee', borderRadius: '4px' }}></div>
                  )}
                  <div>
                    <strong style={{ display: 'block', fontSize: '0.95rem' }}>{s.name || 'Без названия'}</strong>
                    <a href={s.url} target="_blank" rel="noreferrer" style={{ fontSize: '0.85rem', color: '#0066cc' }}>{s.url || 'Нет ссылки'}</a>
                  </div>
                </div>
              )) : (
                <p style={{ color: '#888' }}>Соцсети не добавлены</p>
              )}
            </div>
          </div>
        ) : (
          <form className="admin-form" onSubmit={handleSave} style={{ maxWidth: '800px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
              {contacts.socials?.map((s, idx) => (
                <div key={s.id} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', padding: '1rem', border: '1px solid #e5e5e5', borderRadius: '4px', background: '#fafafa' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
                    {s.iconUrl ? (
                      <img src={s.iconUrl} alt="" style={{ width: '48px', height: '48px', objectFit: 'contain', border: '1px solid #ccc', background: '#333', borderRadius: '4px' }} />
                    ) : (
                      <div style={{ width: '48px', height: '48px', border: '1px dashed #ccc', background: '#fff', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', color: '#888' }}>Нет иконки</div>
                    )}
                    <label style={{ cursor: 'pointer', fontSize: '0.75rem', color: '#111', textDecoration: 'underline' }}>
                      Загрузить
                      <input 
                        type="file" 
                        accept="image/*" 
                  ...
                  <button 
                    ...
                    style={{ background: 'none', border: 'none', color: '#b91c1c', cursor: 'pointer', padding: '0.5rem', alignSelf: 'center' }}
                    title="Удалить"
                  >
                    ✕
                  </button>
            ...
            <div style={{ paddingTop: '1.5rem', borderTop: '1px solid #e5e5e5', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
              <button
                type="button"
                className="admin-btn"
