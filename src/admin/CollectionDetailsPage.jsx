    ...
    <div className="admin-page">
      <style>
        {`
          ...
          .icon-btn { background: var(--color-core-white); border: none; border-radius: 50%; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--color-core-black); transition: transform 0.1s, background 0.2s; text-decoration: none; padding: 0; }
          .icon-btn:hover { transform: scale(1.1); background: var(--color-ui-bg-light); }
      ...
      {showForm ? (
        ...
        <div style={{ background: 'var(--color-core-white)', padding: '1.5rem', borderRadius: '8px', marginBottom: '2rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', gap: '2rem' }}>
          {collection.image_url ? (
            <div className="image-overlay-container" style={{ width: '200px', height: '200px', flexShrink: 0, borderRadius: '8px', overflow: 'hidden', background: 'var(--color-ui-bg-light)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ width: '200px', height: '200px', flexShrink: 0, borderRadius: '8px', background: 'var(--color-ui-bg-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-core-dark-grey)' }}>
              Нет фото
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '0.5rem 1rem', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
            <div style={{ color: 'var(--color-core-dark-grey)' }}>Сезон:</div>
            <div>{collection.season_name || '—'}</div>
            
            <div style={{ color: 'var(--color-core-dark-grey)' }}>Статус:</div>
            <div>
            ...
            <div style={{ color: 'var(--color-core-dark-grey)' }}>Порядок:</div>
            <div>{collection.sort_order}</div>
            
            <div style={{ color: 'var(--color-core-dark-grey)' }}>Категории:</div>
          ...
          {collection.description && (
            <div>
              <div style={{ color: 'var(--color-core-dark-grey)', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Описание:</div>
      ...
      {/* Catalogs Section */}
      <div style={{ background: 'var(--color-core-white)', padding: '1.5rem', borderRadius: '8px', marginBottom: '2rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            {catalogs.map(cat => (
              <div key={cat.id} style={{ 
                ...
                borderRadius: '6px',
                textAlign: 'center',
                background: !cat.published ? 'var(--color-ui-bg-light)' : 'var(--color-ui-bg-light)',
                ...
                <div style={{ height: '140px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-ui-bg-light)', borderRadius: '4px', overflow: 'hidden' }}>
                  {cat.image_url ? (
                    <img src={cat.image_url} alt="" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                  ) : (
                    <span style={{ fontSize: '0.7rem', color: 'var(--color-core-dark-grey)' }}>Нет обложки</span>
                  )}
                </div>
                ...
                <div style={{ marginTop: 'auto' }}>
                  {isEditingCatalogs ? (
                    <input 
                      ...
                      style={{ fontSize: '0.75rem', padding: '0.25rem', width: '100%', boxSizing: 'border-box', border: '1px solid var(--color-core-light-grey)', borderRadius: '4px' }}
                    />
                  ) : cat.pdf_url ? (
            ...
            {categories.map(cat => {
              ...
              return (
                <div key={cat.id} style={{ 
                  ...
                  borderRadius: '6px',
                  textAlign: 'center',
                  background: isHidden ? 'var(--color-ui-bg-light)' : 'var(--color-ui-bg-light)',
                  ...
                  <div className={cat.image_url ? "image-overlay-container" : ""} style={{ height: '90px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-ui-bg-light)', borderRadius: '4px' }}>
                    {cat.image_url ? (
                      <>
                        <img src={cat.image_url} alt="" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                        <div className="image-overlay">
                      ...
                      <span style={{ fontSize: '0.7rem', color: 'var(--color-core-dark-grey)' }}>Нет фото</span>
                    )}
                  </div>
                  ...
                  {isEditingCategories && (
                    hasCustomImage ? (
                      <button 
                        type="button" 
                        className="admin-btn" 
                        style={{ padding: '0.25rem 0.5rem', fontSize: '0.7rem', width: '100%', color: 'var(--color-core-black)' }}
                  ...
                  {hasCustomImage && !isEditingCategories && (
                    <div style={{ fontSize: '0.65rem', color: 'var(--color-core-dark-grey)', marginTop: '0.25rem' }}>Свое фото</div>
                  )}
                </div>
              )
            })}
      ...
      {/* Products Section */}
      <div style={{ background: 'var(--color-core-white)', padding: '1.5rem', borderRadius: '8px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
